...
Usage:
    python3 scripts/update_sheet.py --input /tmp/js3_filtered.json
...
CONFIG_PATH = os.path.expanduser("~/.config/job-search-3/config.json")
GOG_CRED_PATH = os.path.expanduser("~/.config/gogcli/credentials.json")
GOG_TOKEN_PATH = os.path.expanduser("~/.config/job-search-3/gog_token.json")

SS_ID = "1KVpP2Ipq6wwrvuy2O0JRXH4SBKIDAL6buBGEU90mkj4"
SHEET_NAME = "Jobs"
NCOLS = 8
...
def get_access_token():
    ...
    # Find the gog token for arajan.bot@gmail.com (or whichever account)
    token_path = GOG_TOKEN_PATH
    if not os.path.exists(token_path):
        # Fall back to finding any exported token
        import glob
        candidates = glob.glob("/tmp/gog_token*.json")
        if candidates:
            token_path = candidates[0]
        else:
            print("No gog token found. Export one with: gog auth tokens export ... --out " + GOG_TOKEN_PATH, file=sys.stderr)
            sys.exit(1)
